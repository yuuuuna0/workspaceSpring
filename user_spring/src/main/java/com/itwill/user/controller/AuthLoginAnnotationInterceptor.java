...
public class AuthLoginAnnotationInterceptor implements HandlerInterceptor {
	...
	// preHandle() : 컨트롤러 메소드보다 먼저 수행되는 메서드
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		System.out.println("### AuthLoginAnnotationInterceptor.preHandle()메써드");
		/*		<< handler 객체 >>
		 * Controller method 정보를 가지고 있는 reflection 객체
		[ 핸들러 메소드(HandlerMethod)란? ]
			HandlerMethod는 @RequestMapping이 붙은 메소드의 정보를 추상화한 객체이다. 
			HandlerMethod는 그 자체가 실행가능한 객체가 아니라(cf. Controller객체가 실행 가능한 객체) 
			메소드를 실행하기 위해 필요한 참조정보를 담고 있는 객체 로써 다음과 같은 정보들을 가지고 있다.
...
 
		...
		*************************************************************************************/   
		...
		//handler메소드 안: annotation 붙어있는 것들만 확인한다.
